...
function OrderReportPage() {
  ...
  // Fetch details for a specific order (including its items)
  const fetchOrderDetails = async (orderId) => {
    try {
      const response = await axios.get(`/api/ordersreport/${orderId}`);
      console.log('Order details:', response.data);
      setSelectedOrder(response.data); // Store the fetched order details
      setShowModal(true); // Show the modal after fetching order details
    } catch (error) {
  ...
  // Calculate total price for the selected order's items
  ...
  return (
    <div className="container">
      <h1 style={{ fontWeight: 'bold' }}>Sales Report</h1>
